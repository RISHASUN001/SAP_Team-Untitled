...
**How would you like to get started today?**`,
  ...
  const generateAIResponse = async (
    ...
    if (mode === "mentor") {
      ...
      try {
        const res = await fetch("http://localhost:5001/api/mentor-suggest", {
          method: "POST",
    ...
    if (mode === "practice") {
      ...
      try {
        const res = await fetch(
          "http://localhost:5002/chat/api/practice-respond",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
    ...
    try {
      const res = await fetch("http://localhost:5003/api/onboarding-chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          message: userMessage,
...
What aspect would you like to know more about?`,
  ...
  const startPracticeSession = async () => {
    try {
      const res = await fetch("http://localhost:5002/api/practice-start", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          user_id: currentUser?.id || "default_user",
...
**What would you like to discuss?**`,
    ...
    try {
      ...
      await fetch("http://localhost:5002/api/practice-reset", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ user_id }),
      });
      ...
      await fetch("http://localhost:5003/api/onboarding-reset", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ user_id }),
      });
                    ...
                    <div className="prose prose-sm max-w-none dark:prose-invert">
                      <ReactMarkdown
                        components={{
                          ...
                          h3: ({ children }) => (
                            <h3 className="font-semibold text-lg text-gray-900 dark:text-white mb-2 mt-4 first:mt-0">
                              {children}
                            </h3>
                          ),
                          h4: ({ children }) => (
                            <h4 className="font-medium text-gray-900 dark:text-white mb-2 mt-3 first:mt-0">
                              {children}
                            </h4>
                          ),
                          code: ({ children }) => (
